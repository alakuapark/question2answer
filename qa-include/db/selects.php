...
*/
	{
		...
		if ($full) {
			...
			$selectspec['columns']['lastip']='IpToString(^posts.lastip)';
			$selectspec['columns'][]='^posts.parentid';
			$selectspec['columns']['lastviewip']='IpToString(^posts.lastviewip)';
		}

		if ($user) {
			$selectspec['columns'][]='^posts.userid';
			$selectspec['columns'][]='^posts.cookieid';
			$selectspec['columns']['createip']='IpToString(^posts.createip)';
...
*/
	{
		...
		$selectspec['columns']['oip']='IpToString('.$poststable.($fromupdated ? '.lastip' : '.createip').')';
...
*/
	{
		...
		$selectspec['source'].=" JOIN (SELECT postid FROM ^posts WHERE ".
			...
			(isset($createip) ? "createip=StringToIp($) AND " : "").
...
*/
	{
		...
		$selectspec['source'].=" JOIN ^posts AS parentposts ON".
			...
			" JOIN (SELECT postid FROM ^posts WHERE ".
			qa_db_categoryslugs_sql_args($categoryslugs, $selectspec['arguments']).
			(isset($lastip) ? "lastip=StringToIp($) AND " : "").
...
*/
	{
		return array(
			'columns' => array(
				...
				'loginip' => 'IpToString(loginip)', 'written' => 'UNIX_TIMESTAMP(written)', 'writeip' => 'IpToString(writeip)',
...
*/
	{
		return array(
			'columns' => array('action', 'period', 'count'),
			'source' => '^iplimits WHERE ip=COALESCE(StringToIp($), 0)',
			'arguments' => array($ip),
			'arraykey' => 'action',
		);
	}
