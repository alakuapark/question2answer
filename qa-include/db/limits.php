...
*/
	{
		...
		if (isset($ip)) {
			$selects[]="(SELECT 'ip' AS limitkey, period, count FROM ^iplimits WHERE ip=COALESCE(StringToIp($), 0) AND action=$)";
			$arguments[]=$ip;
			$arguments[]=$action;
		}
...
*/
	{
		qa_db_query_sub(
			'INSERT INTO ^iplimits (ip, action, period, count) VALUES (COALESCE(StringToIp($), 0), $, #, #) '.
			'ON DUPLICATE KEY UPDATE count=IF(period=#, count+#, #), period=#',
