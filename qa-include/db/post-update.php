...
*/
	{
		...
		if (isset($selchildid) && isset($lastuserid) && isset($lastip))
			qa_db_query_sub(
				"UPDATE ^posts SET updated=NOW(), updatetype=$, lastuserid=$, lastip=StringToIp($) WHERE postid=#",
...
*/
	{
		if (isset($lastuserid) || isset($lastip)) {
			qa_db_query_sub(
				"UPDATE ^posts SET closedbyid=#, updated=NOW(), updatetype=$, lastuserid=$, lastip=StringToIp($) WHERE postid=#",
...
*/
	{
		if (isset($lastuserid) || isset($lastip)) {
			qa_db_query_sub(
				'UPDATE ^posts SET type=$, updated=NOW(), updatetype=$, lastuserid=$, lastip=StringToIp($) WHERE postid=#',
...
*/
	{
		if (isset($lastuserid) || isset($lastip))
			qa_db_query_sub(
				"UPDATE ^posts SET parentid=#, updated=NOW(), updatetype=$, lastuserid=$, lastip=StringToIp($) WHERE postid=#",
...
*/
	{
		if (isset($lastuserid) || isset($lastip)) // use COALESCE() for name since $name=null means it should not be modified (for backwards compatibility)
			qa_db_query_sub(
				'UPDATE ^posts SET title=$, content=$, format=$, tags=$, name=COALESCE($, name), notify=$, updated=NOW(), updatetype=$, lastuserid=$, lastip=StringToIp($) WHERE postid=#',
...
*/
	{
		if (isset($lastuserid) || isset($lastip))
			qa_db_query_sub(
				"UPDATE ^posts SET categoryid=#, updated=NOW(), updatetype=$, lastuserid=$, lastip=StringToIp($) WHERE postid=#",
